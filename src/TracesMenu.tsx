...
import { AbundanceTreeLayout, DestructionTreeLayout, ElationTreeLayout, HarmonyTreeLayout, HuntTreeLayout, NihilityTreeLayout, PreservationTreeLayout, RemembranceTreeLayout } from "./types/hsr";
import "./TracesMenu.css";
import { abundanceImg, destructionImg, elationImg, harmonyImg, huntImg, nihilityImg, preservationImg, remembranceImg } from "./assets/paths";
  ...
  return (
    <div className="hsr-traces-container">
      ...
      <div className="hsr-traces-canvas" style={{transform: `scale(${scale})`}}>
        ...
        { bgWatermark === huntImg && (
            HuntTreeLayout.connections.map((conn, idx) => {
                const start = HuntTreeLayout.positions[conn.from];
                const end = HuntTreeLayout.positions[conn.to];
                if (!start || !end) return null;
        ...
        { bgWatermark === preservationImg && (
            PreservationTreeLayout.connections.map((conn, idx) => {
                const start = PreservationTreeLayout.positions[conn.from];
                const end = PreservationTreeLayout.positions[conn.to];
                if (!start || !end) return null;
        ...
        <div className="hsr-traces-nodes-layer">
            ...
            {bgWatermark === huntImg && (
                Object.entries(HuntTreeLayout.positions).map(([id, pos]) => {
                    const nodeData = nodes[id];
                    if (!nodeData) return null;
                    const isSelected = selectedNodeId === id;
            ...
            {bgWatermark === preservationImg && (
                Object.entries(PreservationTreeLayout.positions).map(([id, pos]) => {
                    const nodeData = nodes[id];
                    if (!nodeData) return null;
                    const isSelected = selectedNodeId === id;
